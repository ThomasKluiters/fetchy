...
class Fetchy(object):
    ...
    def build_repository(self):
        repository = fty.get_packages_control_file(
          self.config.distribution, self.config.version, self.config.architecture, self.config.mirror
        )
        ...
        for ppa in self.config.ppas:
            ppa_repository = fty.get_packages_control_file(
                self.config.distribution, self.config.version, self.config.architecture, ppa=ppa
            )
    ...
    def extract_packages(self, extract_dir, packages_to_extract):
        ...
        downloaded_packages = self.download_packages(temp_download_dir, packages_to_extract)

        fty.Extractor(extract_dir).extract_all(
          downloaded_packages
        )
